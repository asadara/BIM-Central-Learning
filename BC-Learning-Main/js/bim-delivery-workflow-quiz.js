/**
 * BIM Delivery Workflow Quiz Logic
 * All-at-once quiz format for BIM Delivery Workflow understanding
 * Features: randomization, scoring, review mode, localStorage persistence
 */

class BIMDeliveryWorkflowQuiz {
    constructor() {
        this.questions = [];
    ...
    async loadQuestions() {
        try {
            const response = await fetch('/js/bim-delivery-workflow-quiz-bank.json');
            if (!response.ok) {
            ...
            console.log(`Loaded ${this.questions.length} questions from BIM Delivery Workflow bank`);
        } catch (error) {
    ...
    checkPreviousAttempt() {
        const lastAttempt = localStorage.getItem('bim-delivery-workflow-quiz-last-attempt');
        if (lastAttempt) {
    ...
    showResults() {
        ...
        // Save attempt to localStorage and sync to backend
        this.saveAttempt();
    ...
    async saveAttempt() {
        ...
        localStorage.setItem('bim-delivery-workflow-quiz-last-attempt', JSON.stringify(attempt));
        ...
        try {
            ...
            const quizResult = {
                ...
                quizName: 'BIM Delivery Workflow Quiz',
                quizCategory: 'workflow',
                sourceType: 'quiz',
            ...
            console.warn('Delivery workflow quiz sync skipped:', error.message);
        }
    }
