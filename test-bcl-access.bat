...
echo [3/4] Checking backend server (port 5051)...
powershell -NoProfile -Command "try { $tcp = New-Object System.Net.Sockets.TcpClient; $tcp.Connect('127.0.0.1', 5051); echo '✅ Backend port 5051 accessible'; $tcp.Close() } catch { echo '❌ Backend port 5051 not accessible' }" 2>nul
echo.

echo [4/4] Testing HTTP access to bcl.nke.net...
powershell -NoProfile -Command "try { $response = Invoke-WebRequest -Uri 'http://bcl.nke.net' -Method Head -TimeoutSec 5 -MaximumRedirection 0; echo '✅ HTTP access successful'; echo '   Status:' $response.StatusCode } catch { if ($_.Exception.Response.StatusCode -eq 302) { echo '⚠️  Redirect detected - check HSTS cache'; echo '   Solution: Clear HSTS in chrome://net-internals/#hsts' } else { echo '❌ HTTP access failed:'; echo '   Error:' $_.Exception.Message } }" 2>nul
echo.
